...
} from '../../types/nexusMods/nexusMods.api.types.js';
import {
  NEXUS_MODS_API_APIKEY_HEADER,
  NEXUS_MODS_API_BASE_URL,
  NEXUS_MODS_API_GAME_DOMAIN_NAME,
} from '../../const.js';
...
export class NexusModsAPI {
  ...
  private async callApi<P extends Path, M extends PathMethod<P>, T>(
    ...
    const { path: pathParams } = params[0]?.path != null ? params[0] : { path: {} };
    ...
    if (response.status !== 200) {
      throw new Error(
        `Error occurred while calling ${response.config.url}: ${response.statusText}`
      );
    }
  ...
  private static _convertUrlWithNamedParams(
    url: string,
    ...
    if (malformed.length > 0) {
      throw new Error(
        `Malformed request: the path params ${JSON.stringify(malformed)} were found in the URL but had no matching values in pathParams`
      );
    }
