...
const unpackMods = async (blockUuid: string): Promise<UnpackResult | null> => {
  ...
  LOGGER.log(
    chalk.dim.yellow(`Creating unpack merged dir for this block at ${unpackMergedDirPath}`)
  );
  ...
  LOGGER.log(
    `${logPrefix}Unpacking mods in path ${unpackModsDirPath} for block ${uninstalledBlock.uuid} and copying files to ${unpackDirPath}`
  );
